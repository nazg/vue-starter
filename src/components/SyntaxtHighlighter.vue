...
<template>
  <pre><code ref="code" v-bind:class="`language-${lang}`"/></pre>
</template>

<script lang="ts">
  import Prism from "prismjs"
  import "prismjs/components/prism-json.js" // eslint-disable-line sort-imports
  ...
  export default Vue.extend({
    ...
    methods: {
      highlight(): void {
        const elem = this.$refs.code as HTMLElement
        if (typeof this.value === "string") {
          elem.textContent = this.value
        }
        else {
          try {
            elem.textContent = JSON.stringify(this.value, null, 2)
          }
          catch (err) {
...
<style lang="scss" scoped>
  pre[class*="language-"] { border-radius: 0; border: 0; margin: 1px; }
  // import is not working in electron, so we include inline here the theme
